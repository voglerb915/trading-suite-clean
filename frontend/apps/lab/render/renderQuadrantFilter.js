// apps/lab/render/renderQuadrantFilter.js
...
export function renderQuadrantFilterBar(containerId, onFilterChange) {
    ...
    // State initialisieren: Standardmäßig alle Quadranten aktiv
    ...
    allBtn.addEventListener("click", () => {
        ...
        GlobalState.set("activeQuadrants", newSet);
        renderQuadrantFilterBar(containerId, onFilterChange);
        if (onFilterChange) onFilterChange();
    });
    container.appendChild(allBtn);

    // Einzelne Quadranten-Pillen (Single-Select Logik analog zu Sektoren)
    QUADRANTS.forEach(q => {
